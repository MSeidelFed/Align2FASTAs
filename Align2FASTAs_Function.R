...
FASTA_source <- read.fasta(file = FASTA_source_directory, seqtype = "AA", as.string = T, whole.header = T, seqonly = F)
...
FASTA_target <- read.fasta(file = FASTA_target_directory, seqtype = "AA", as.string = T, whole.header = T, seqonly = F)
...
for (i in 1:dim(character_FASTA_target)[1]) {
  ...
  alignment <- pairwiseAlignment(pattern = character_FASTA_source[,1],
